...
class InventoryUtils{
	...
	/**
	 * @param DesktopPlayer $player onWindowClick
	 */
	...
	public function onWindowClick(ClickWindowPacket $packet) : ?InventoryTransactionPacket{
		...
		if($accepted){
			...
			$pk->trData->getTypeId = InventoryTransactionPacket::TYPE_NORMAL;
	...
	public function onCreativeInventoryAction(CreativeInventoryActionPacket $packet) : ?DataPacket{
		if($packet->slot === 65535){
			...
			if($packet->slot === -1){//DropItem
				...
				if($oldItem instanceof ComputerItem){//ComputerItem looolll
					$action = $this->addNetworkInventoryAction3(NetworkInventoryAction::SOURCE_CONTAINER, ContainerIds::ARMOR, $inventorySlot, $oldItem, $newItem);
				}elseif($newItem instanceof ComputerItem){
					$action = $this->addNetworkInventoryAction3(NetworkInventoryAction::SOURCE_CONTAINER, ContainerIds::ARMOR, $inventorySlot, $oldItem, $newItem);
				}elseif($inventorySlot instanceof ComputerItem){
					$action = $this->addNetworkInventoryAction3(NetworkInventoryAction::SOURCE_CONTAINER, ContainerIds::ARMOR, $inventorySlot, $oldItem, $newItem);
					//ItemStackWrapper
				}elseif($oldItem instanceof ItemStackWrapper){
					$action = $this->addNetworkInventoryAction2(NetworkInventoryAction::SOURCE_CONTAINER, ContainerIds::ARMOR, $inventorySlot, $oldItem, $newItem);
				}elseif($newItem instanceof ItemStackWrapper){
					$action = $this->addNetworkInventoryAction2(NetworkInventoryAction::SOURCE_CONTAINER, ContainerIds::ARMOR, $inventorySlot, $oldItem, $newItem);
				}elseif($inventorySlot instanceof ItemStackWrapper){
					$action = $this->addNetworkInventoryAction2(NetworkInventoryAction::SOURCE_CONTAINER, ContainerIds::ARMOR, $inventorySlot, $oldItem, $newItem);
				} else {
	...
	public function addNetworkInventoryAction2(int $sourceType, int $windowId, int $inventorySlot, ItemStackWrapper $oldItem, ItemStackWrapper $newItem) : NetworkInventoryAction{
		$action = new NetworkInventoryAction();
		$action->sourceType = $sourceType;
		$action->windowId = $windowId;
		$action->inventorySlot = $inventorySlot;
	...
	public function addNetworkInventoryAction3(int $sourceType, int $windowId, int $inventorySlot, ComputerItem $oldItem, ComputerItem $newItem) : NetworkInventoryAction{
		$action = new NetworkInventoryAction();
		$action->sourceType = $sourceType;
		$action->windowId = $windowId;
		$action->inventorySlot = $inventorySlot;
